...
module.exports = async (client, discord, member) => {
  ...
  const background = await loadImage(join(__dirname, "../../img", "neon.jpg")); //Imagen de fondo
  ...
  const rol = guild.roles.cache.find((role) => role.name === "putos");
  member.roles.add(rol);
  //& ROL

  const channel = member.guild.channels.cache.find(
    (channel) => channel.id === "907111784274624512"
  );
